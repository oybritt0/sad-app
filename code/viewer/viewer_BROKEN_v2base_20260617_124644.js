...
// Building-type filter — shows buildings by their OSM `building`-tag program.
...
function renderBuildings(g, gj) {
  ...
  // Building-type filter — hide programs the user unchecked. Anchors are
...
// Export tuning — keep the exported SVG small enough for Illustrator to open.
const EXPORT_SAT_MAX_EDGE = 2400;      // px — cap embedded satellite long edge
...
function roundSvgCoords(node) {
  ...
  // Geometry only — NOT stroke-width / font-size, so hairlines/type stay intact.
