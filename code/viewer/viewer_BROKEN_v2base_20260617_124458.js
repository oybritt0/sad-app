...
// POI surfaces are driven by POI_CATEGORIES, NOT ROSSETTI_ORDER. Residential is
// deliberately excluded here: Overture residential POIs are too sparse/inconsistent
// to map honestly. Residential is sourced from the OSM `building` tag at the
// building level (see buildingProgram()), so it stays in ROSSETTI_ORDER for the
// building-program legend but is removed from every POI surface (filter, heatmap,
// dots, on-page legend, export, count).
...
const state = {
  ...
  // Driven by buildingProgram() (the OSM `building` tag), incl. residential.
...
// Building-type filter — shows buildings on the map by their OSM `building`-tag
// program (residential, office, retail, sport, hotel, parking, open_space, other).
// Injected as a sidebar sub-block (not a numbered section) anchored to the
// building controls; reuses .category-checks / .sw-dot / .bulk-actions styling.
function buildBuildingTypeFilter() {
  ...
  if (!wrap) {
    ...
    hint.textContent = 'Type colors show in Dominant POI mode';
    hint.style.cssText = 'font-size:10px;color:var(--ink-faint);margin-top:6px;';
    wrap.appendChild(hint);
    // Placement: after the building color-mode controls, else before the POI
    // filter, else appended to the sidebar. All anchors are referenced by
    // viewer.js elsewhere, so at least one is guaranteed to exist.
...
function renderBuildings(g, gj) {
  ...
  // Building-type filter — hide programs the user has unchecked. Anchors are
  // sport-class, so they follow the 'sport' toggle.
  const bProgOn = state.buildingProgramOn || {};
  feats = feats.filter(d => {
    const prog = sadStadia.has(d) ? 'sport' : buildingProgram(d.properties || {});
...
// Export tuning — these keep the exported SVG small enough that Illustrator can
// open it. A full-res PNG satellite + full-precision coordinates is what crashes
// it; capping the raster (JPEG) and rounding coordinates are the two fixes.
const EXPORT_SAT_MAX_EDGE = 2400;      // px — cap the embedded satellite long edge
...
function imageUrlToDataUri(url) {
  return new Promise(resolve => {
    ...
    im.onload = () => {
      clearTimeout(timer);
      try {
        // Cap the long edge and encode as JPEG. A full-resolution PNG of tiled
        // satellite imagery is tens of MB of base64 and crashes Illustrator on
        // import; a capped JPEG is ~10-20x smaller and embeds cleanly (imagery
        // is photographic, so JPEG is the right format and artifacts are moot).
...
// thousands of building paths + POI circles balloon the file and choke
// Illustrator's parser. Rounding projected PIXEL coordinates to a couple of
// decimals is sub-pixel (visually lossless) and typically shrinks the SVG by
// 50-70%. Mirrors the standalone shrink_svg.py so the in-app export is safe too.
function roundSvgCoords(node) {
  ...
  // Geometry attributes only — deliberately NOT stroke-width / font-size, so
  // hairlines and small type are never thinned away by rounding.
