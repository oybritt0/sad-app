...
const CHARTS=[['typology',cTypology],['census',cCensus],['table',cTable],['benchmark',cBench],['rose',cRose],['similarity',cSim],['program',cProgram],['scatter',cScatter],['ranking',cRank]];
...
function extract(m){
  ...
  for(const r of (m.sads||[])){
    ...
    rows.push({sad_id:r.sad_id,name:(parts[1]||r.sad_id).replace(/-/g,' '),city:(parts[2]||'').replace(/-/g,' '),typology:r.typology,
      ...
      walk:g(r,'walkshed','walkshed_10min_acres'),nodes:g(r,'centrality','network_size','nodes'),boundary:g(r,'artifacts','sad_boundary'),program_real:(r.program_real||null)});
  }
