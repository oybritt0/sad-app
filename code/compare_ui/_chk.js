...
const PROGOCC_C={residential:'#5fae7e',hotel_institutional:'#c98a3a',retail:'#e2674f',office:'#5688c4',entertainment:'#d2a23f',industrial_service:'#8a7a6a',civic_medical:'#7c9c8f',parking:'#8a8a8a',other:'#d6cebf'};
const PROGOCC_LABEL={residential:'residential',hotel_institutional:'hotel / institutional',retail:'retail',office:'office',entertainment:'entertainment',industrial_service:'industrial / service',civic_medical:'civic / medical',parking:'parking',other:'other'};
...
function cProgram(){
  const cats=Object.keys(PROGOCC_C),s=sel();const W=800,top=74,rowH=50,pad=20,bw=W-2*pad;const H=top+s.length*rowH+78;let inner='';
  s.forEach((id,i)=>{const y=top+i*rowH,r=byId[id];
    const raw=r._raw||{};
    let shares={},sum=0,avail=false;
    cats.forEach(c=>{const v=+raw['progocc_pct_'+c]; if(isFinite(v)){shares[c]=v;sum+=v;avail=true;}});
    // values are percent (0-100); normalise to fraction of the present sum
    if(avail && sum>0){ cats.forEach(c=>{shares[c]=(shares[c]||0)/sum;}); }
    const flag=avail?(Math.round((shares.parking||0)*100)+'% parking'):'no occupancy data';
    ...
    if(avail){let cx=pad;cats.forEach(c=>{const v=+shares[c]||0;if(v<=0)return;const w=v*bw;inner+='<rect x="'+cx.toFixed(1)+'" y="'+(y+20)+'" width="'+w.toFixed(1)+'" height="18" fill="'+PROGOCC_C[c]+'"><title>'+PROGOCC_LABEL[c]+' '+Math.round(v*100)+'%</title></rect>';cx+=w;});}
  ...
  let ky=top+s.length*rowH+12,kx=pad;cats.forEach(c=>{inner+='<rect x="'+kx+'" y="'+ky+'" width="9" height="9" rx="2" fill="'+PROGOCC_C[c]+'"/><text x="'+(kx+13)+'" y="'+(ky+8)+'" class="axlab">'+PROGOCC_LABEL[c]+'</text>';kx+=18+PROGOCC_LABEL[c].length*5.6+16;if(kx>W-140){kx=pad;ky+=18;}});
  const sub='building program by NSI occupancy, area-weighted \u00b7 parking split via POI \u00b7 sport-blind';
  return {title:'Program mix \u2014 occupancy',svg:svg(W,Math.max(H,ky+20),'Program mix \u2014 occupancy',sub,inner)};
}
function cScatter(){
...
const CHARTS=[['census',cCensus],['table',cTable],['rose',cRose],['indices',cIndices],['delta',cDelta],['fingerprint',cFingerprint],['matrix',cMatrix],['maps',cMaps],['program',cProgram],['daynight',cDayNight],['scatter',cScatter],['ranking',cRank]];
...
function render(){
  ...
  CHARTS.forEach(([key,fn])=>{let out;try{out=fn();}catch(err){out={title:key,svg:'<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 800 90\"><rect width=\"800\" height=\"90\" fill=\"#ffffff\"/><text x=\"20\" y=\"34\" style=\"font:700 14px Arial;fill:#d64545\">'+key+' panel error</text><text x=\"20\" y=\"58\" style=\"font:11px monospace;fill:#5a5a5a\">'+String(err&&err.message||err).slice(0,90)+'</text></svg>'};}LAST[key]=out.svg;
    ...
    else if(key==='program')ax='<span class="axlab" style="color:#8a8a8a">NSI occupancy, area-weighted</span>';
...
// --- Analogue entry: embedding composite lenses + per-family weights + target typology ---
let AE_FN=[], AE_VEC={}, AE_METRIC='cosine', AE_TARGET_TYP=null;
const AE_LENSES=[
  { key:'current', title:'Current condition', q:'Places like the site now', typFilter:false,
    note:'Connectivity, built form, demographics & economics (present levels), z-scored embedding. No typology filter.' },
  { key:'trajectory', title:'Trajectory', q:'Similar change over time', typFilter:false,
    note:'Income, home-value, population & ownership trend (2013-2023). Narrow lens; means similar rate/direction of change.' },
  { key:'target', title:'Target character', q:'References for the intended character', typFilter:true,
    note:'Built form, NSI occupancy program, anchor. Filtered to the chosen target typology. Anchor is zero for many districts.' },
];
// families within each lens (embedding feature prefixes)
const AE_FAMILIES={
  current:[
    {key:'conn',label:'Connectivity',match:f=>f.indexOf('conn_')===0},
    {key:'form',label:'Built form',match:f=>f.indexOf('morph_')===0},
    {key:'demo',label:'Demographics',match:f=>['demo_pop_density','demo_median_age','demo_pct_owner_occupied'].indexOf(f)>=0},
    {key:'econ',label:'Economics',match:f=>['econ_median_income','econ_median_home_value','econ_median_gross_rent'].indexOf(f)>=0},
  ],
  trajectory:[
    {key:'econtr',label:'Economic trend',match:f=>f.indexOf('econ_')===0&&f.slice(-6)==='_trend'},
    {key:'demotr',label:'Demographic trend',match:f=>f.indexOf('demo_')===0&&f.slice(-6)==='_trend'},
  ],
  target:[
    {key:'form',label:'Built form',match:f=>f.indexOf('morph_')===0},
    {key:'prog',label:'Program',match:f=>f.indexOf('progocc_')===0},
    {key:'anchor',label:'Anchor',match:f=>f.indexOf('anchor_')===0},
  ],
};
...
const AE_METHOD={
  ...
  current:{title:'Current condition \u2014 the site as it exists',
    ...
    cav:'Drawn/synthetic and data-incomplete districts are excluded. A weight of 0 removes a family entirely.'},
  trajectory:{title:'Trajectory \u2014 the journey, not the snapshot',
    src:'Module 8 \u00b7 income / home-value / population / ownership trend (2013-2023)',
    body:'Ranks districts by similarity in how they changed over 2013-2023. Weight economic vs demographic change to focus the comparison. Profile similarity means a similar pattern of change; magnitude means a similar amount.',
    cav:'A narrow signal (few measures); high percentages should not be read as broad confidence. US districts only.'},
  target:{title:'Target character \u2014 the form you are aiming for',
    src:'Module 8 \u00b7 built form + occupancy program + anchor',
    body:'Filters the corpus to the chosen target typology, then ranks those by similarity in built form, occupancy program, and anchor. Set the target character to the district type you intend to create \u2014 which need not match the study district today.',
    cav:'Anchor geometry is zero for many districts. Typology labels come from the corpus classification.'},
};
function _aeCos(a,b){let d=0,na=0,nb=0;for(let i=0;i<a.length;i++){d+=a[i]*b[i];na+=a[i]*a[i];nb+=b[i]*b[i];}return(na===0||nb===0)?0:d/Math.sqrt(na*nb);}
...
function _aeWeightedSub(vec,cols,wsqrt){const o=new Array(cols.length);for(let i=0;i<cols.length;i++)o[i]=vec[cols[i]]*wsqrt[i];return o;}
function aeAnalogues(lens,k){
  const cols=lens._cols; const q=AE_VEC[FOCUS]; if(!q||!cols.length) return [];
  // per-col sqrt(weight) from this lens's family weights
  const wmap=AE_WEIGHTS[lens.key]||{};
  const wsqrt=lens._colFam.map(fk=>Math.sqrt(Math.max(0,(wmap[fk]!=null?wmap[fk]:1))));
  const qv=_aeWeightedSub(q,cols,wsqrt);
  const ftyp = lens.typFilter ? AE_TARGET_TYP : null;
  const out=[];
  for(const id in AE_VEC){
    if(id===FOCUS||_aeDrawn(id)) continue;
    if(lens.typFilter && ftyp && (byId[id]||{}).typology!==ftyp) continue;
    const sv=_aeWeightedSub(AE_VEC[id],cols,wsqrt);
...
function renderAnaEntry(){
  ...
  if(AE_TARGET_TYP==null && byId[FOCUS]) AE_TARGET_TYP=byId[FOCUS].typology||null;
  host.innerHTML='';
  const typs=[...new Set(Object.keys(AE_VEC).map(id=>(byId[id]||{}).typology).filter(Boolean))].sort();
  for(const lens of AE_LENSES){
    const res=aeAnalogues(lens,6);
    const col=document.createElement('div'); col.className='ae-lens';
    let headRight='<span class="ae-lm" data-aem="'+lens.key+'">methods</span>';
    if(lens.typFilter){
      headRight='<select class="ae-typ" id="ae-typ-sel">'+typs.map(t=>'<option'+(t===AE_TARGET_TYP?' selected':'')+'>'+t+'</option>').join('')+'</select>'+headRight;
    }
    let rows='';
    if(!res.length){ rows='<div class="ae-note">No analogues for this lens.</div>'; }
      ...
      const sim=AE_METRIC==='cosine'?Math.round(Math.max(0,r.val)*100)+'%':r.val.toFixed(2);
      const nm=((byId[r.id]||{}).name||r.id).replace(/^\d+_/,'').replace(/_/g,' ');
      const tp=(byId[r.id]||{}).typology||'';
      return '<li class="'+(picked?'picked':'')+'" data-add="'+r.id+'"><span class="ae-rk">'+(i+1)+'</span>'+
        '<span class="ae-sim">'+sim+'</span><span class="ae-nm">'+nm+'</span>'+
        '<span class="ae-tp">'+tp+'</span><span class="ae-plus">'+(picked?'\u2212':'+')+'</span></li>';
    }).join('')+'</ul>';
    col.innerHTML='<div class="ae-lh"><div><div class="ae-lt">'+lens.title+'</div>'+
      '<div class="ae-lq">'+lens.q+'</div></div><div class="ae-lhr">'+headRight+'</div></div>'+
      rows+_aeSliders(lens)+'<div class="ae-note">'+lens.note+'</div>';
  ...
  host.querySelectorAll('[data-aew]').forEach(sl=>sl.addEventListener('input',e=>{
    const [lk,fk]=e.target.getAttribute('data-aew').split('|');
    if(!AE_WEIGHTS[lk])aeResetWeights(lk); AE_WEIGHTS[lk][fk]=+e.target.value; renderAnaEntry(); }));
  host.querySelectorAll('[data-aewr]').forEach(b=>b.addEventListener('click',()=>{ aeResetWeights(b.getAttribute('data-aewr')); renderAnaEntry(); }));
  const ts=document.getElementById('ae-typ-sel');
  if(ts) ts.addEventListener('change',e=>{ AE_TARGET_TYP=e.target.value; renderAnaEntry(); });
}
function aeToggle(id){ if(id===FOCUS) return; const i=COMPS.indexOf(id);
  if(i>=0) COMPS.splice(i,1); else { if(COMPS.length>=4) COMPS.shift(); COMPS.push(id); } render(); }
function aeMethod(key){ const m=AE_METHOD[key]; if(!m) return; const c=document.getElementById('ae-mcard');
  c.innerHTML='<button class="x">\u00d7</button><h3>'+m.title+'</h3><div class="src">'+m.src+'</div><p>'+m.body+'</p><p class="cav">'+m.cav+'</p>';
  c.querySelector('.x').addEventListener('click',aeCloseMethod); c.classList.add('show'); document.getElementById('ae-mback').classList.add('show'); }
function aeCloseMethod(){ document.getElementById('ae-mcard').classList.remove('show'); document.getElementById('ae-mback').classList.remove('show'); }
function aeInit(m){
  ...
  AE_LENSES.forEach(L=>{
    const fams=AE_FAMILIES[L.key];
    L._cols=[]; L._colFam=[];
    AE_FN.forEach((f,i)=>{ const fm=fams.find(x=>x.match(f)); if(fm){ L._cols.push(i); L._colFam.push(fm.key); } });
    aeResetWeights(L.key);
  });
  AE_TARGET_TYP=(byId[FOCUS]||{}).typology||null;
