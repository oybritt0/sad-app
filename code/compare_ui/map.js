...
function setBasemap(which) {
  [S.baseLight, S.baseSat, S.baseLabels].forEach(l => { if (l && S.map.hasLayer(l)) S.map.removeLayer(l); });
  if (which === 'sat') { S.baseSat.addTo(S.map); S.baseLabels.addTo(S.map); }
  else S.baseLight.addTo(S.map);
}
...
function renderDrawnPanel(inner) {
  ...
  inner.innerHTML = `
    ...
    <div class="sec">
      <div class="sec-h">Most similar districts</div>
      <div>${matches || '<div class="empty">No matches.</div>'}</div>
      <p class="layer-note" style="margin-left:0">By demographics (ACS). % = similarity, higher is closer. ${a.note || ''}</p>
    </div>
    <div class="actions">
